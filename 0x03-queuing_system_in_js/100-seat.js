...
function reserveSeat(number) {
  cli.set('available_seats', number);
}

async function getCurrentAvailableSeats() {
  const seat = await asyncGet('available_seats');
  return seat;
}
...
app.get('/reserve_seat', function (req, res) {
  ...
  const jobs = que.create('reserve_seat', {'seat': 1}).save((error) => {
    if (error) {
...
app.get('/process', function (req, res) {
    ...
    que.process('reserve_seat', async function(job, done) {
